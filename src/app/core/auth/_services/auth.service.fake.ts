    ...
    return this.getAllUsers().pipe(
      map((result: User[]) => {
        ...
        const user = find(result, (item: User) => {
          return (item.DisplayName.toLowerCase() === email.toLowerCase() && item.UserPasswordDetails.Password === password);
        });
        ...
        user.UserPasswordDetails.Password = undefined;
        return user;
      })
    );
    ...
    return this.http.get(API_USERS_URL).pipe(
      map((users: User[]) => {
        ...
        const user = find(users, (item: User) => {
          return (item.DisplayName.toLowerCase() === email.toLowerCase());
        });
